...
import static com.example.splash.cords.FirebaseCords.MAIN_CHAT_DATABASE2;
...
public class chatGroup2 extends AppCompatActivity {
    @Override
    public void onStart() {
        super.onStart();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_chat_group_2);
    ...
    private void initChatList() {
        ...
        Query query = MAIN_CHAT_DATABASE2.orderBy("timestamp", Query.Direction.DESCENDING);
    ...
    public void addMessage(View view) {
        ...
        if (!TextUtils.isEmpty(message)) {
            ...
            MAIN_CHAT_DATABASE2.document(messageID).set(messageObj).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(chatGroup2.this, "Message Sent", Toast.LENGTH_SHORT).show();
                        chat_box.setText("");
                    } else {
                        Toast.makeText(chatGroup2.this, task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
